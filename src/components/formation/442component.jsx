...
export default function Formation442({ tempDraft, setTempDraft, jornada }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const initialPlayers = [
        ...
        null,
        null,
        null, // Defensas (posiciones 1-4)
        null,
        null,
        null,
        null, // Centrocampistas (posiciones 5-8)
        null,
        null, // Delanteros (posiciones 9-10)
      ]
      ...
      console.log("Jugadores inicializados para 4-4-2:", initialPlayers)
  ...
  const handlePlayerClick = (index, positionId) => {
    ...
    switch (positionId) {
      ...
      case "25": // Defensas
        if (index >= 1 && index <= 4) {
          groupIndex = 6 + (index - 1) // Grupos 6-9 para defensas
        }
        break
      case "26": // Centrocampistas
        if (index >= 5 && index <= 8) {
          groupIndex = 2 + (index - 5) // Grupos 2-5 para centrocampistas
        }
        break
      case "27": // Delanteros
        if (index >= 9 && index <= 10) {
          groupIndex = index - 9 // Grupos 0-1 para delanteros
        }
        break
    }
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto h-[600px]">
      ...
      {/* Defenders - 4 */}
      {renderPlayerIcon(selectedPlayers[1], 1, "25", "75%", "20%")}
      {renderPlayerIcon(selectedPlayers[2], 2, "25", "75%", "40%")}
      {renderPlayerIcon(selectedPlayers[3], 3, "25", "75%", "60%")}
      {renderPlayerIcon(selectedPlayers[4], 4, "25", "75%", "80%")}

      {/* Midfielders - 4 */}
      {renderPlayerIcon(selectedPlayers[5], 5, "26", "50%", "20%")}
      {renderPlayerIcon(selectedPlayers[6], 6, "26", "50%", "40%")}
      {renderPlayerIcon(selectedPlayers[7], 7, "26", "50%", "60%")}
      {renderPlayerIcon(selectedPlayers[8], 8, "26", "50%", "80%")}

      {/* Forwards - 2 */}
      {renderPlayerIcon(selectedPlayers[9], 9, "27", "25%", "35%")}
      {renderPlayerIcon(selectedPlayers[10], 10, "27", "25%", "65%")}
